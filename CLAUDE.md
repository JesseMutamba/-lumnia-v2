...
2. **BUILT — cross-workbook consolidation:** the engagement upload (POST /analyze with an optional `plan` file) merges actuals + projections into one workbook server-side, plan sheets prefixed `PLAN · ` so the plan-pool partition claims them; merged bytes are the stored original (trace stays byte-real; deterministic → dedupe). Monthly mapping pins (mapping.py `month_series`/`build_mapped_monthly`) let the analyst fix ambiguous date-axis roles ("Personnel Production" is payroll, "Camion Citerne (Transfert CPO)" is a truck line); oracles in tests/test_engagement.py and tests/test_mapping.py.
...
4. Logged `# DEBT`: exec KPI tiles don't surface `dropped_outside_axis` (the Production statbar does); mapping.py restricts manual `volume_secondary` to identical axes; yearly-plan-phased-monthly comparison needs an explicit phasing rule to stay honest; compose.py can't print unit suffixes (needs a pipeline-level `unit` field on metrics) and its block headers lack a type ramp. REPAID: the trace endpoint's per-click workbook re-parse (now a content-hash-keyed parsed-grid cache, oracle in test_trace.py).
